...
func SymReconfig() {
	...
	head, tail, err := fetchDetails(masterGrpcClient)
	if err != nil {
		fmt.Println(err)
		return
	}
	...
	go func(pbRaz.MessageBoardClient, chan error) {
		ctx, cancel := context.WithTimeout(context.Background(), time.Second*2)
		defer cancel()
		if _, err := headClient.CreateTopic(ctx, &pbRaz.CreateTopicRequest{Name: "topic12"}); err != nil {
			errCh <- err
		}
	}(headClient, errCh)
